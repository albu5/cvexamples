function [G,evalarr] = baseFiltVec3(I,J,d,smin,smax,window,alpha)
window = window+1;
wini = window(1);
winj = window(2);
wink = window(3);
...
%         display(sprintf('%d|%d, %d|%d',i,di,j,dj));
        for k = 1:size(G,3)
            ...
            [R,S,evalarr(end+1:end+3)] = gaussKernel3(squeeze(J(:,:,i,j,k)), d, smin, smax);
...
%             hist(temp3(:)),pause
            garg = exp(-0.5*temp3).*(1-alpha*temp3);
%             hist(garg(:)), pause(0.5)
            temp4 = garg.*Ipatch;
            res = sum(temp4(:));
            mu = sum(garg(:));
